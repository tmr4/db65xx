...
* Inspect symbols, memory addresses and expressions in Watch pane.  Set the value of symbols and memory ranges when the program is paused.
...
* Evaluate expressions and set symbol values in the Debug Console
...
* Watches can be set for symbols that are in the debug file.  The value of watched symbols can be changed via the `Set Value` context menu item.  Watches can also be set by memory address.  Enter a single address in hex (with a `0x` prefix otherwise it will be interpreted as an expression) to watch the value at that memory location.  The value of a single memory address can not be changed.  Memory ranges can also be watched.  Enter a memory range as the starting and ending address separated with a colon, such as `start:end`.  The range can be expressed either as decimal or hex (with a `0x` prefix).  Memory ranges can be drilled down and individual address values changed with the `Set Value` context menu item.  If you want to *watch and change* a single memory address, enter it as a range, such as `0xe000:0xe000`.  You can then drill down and use the `Set Value` context menu item to change the value at that memory address.
...
5. Expressions haven't been extensively tested.  In most cases an expression is evaluated in two passes.  The first pass replaces any symbols with its address and size.  The second pass replaces these with the value at that location and then the overall expression is evaluated.
